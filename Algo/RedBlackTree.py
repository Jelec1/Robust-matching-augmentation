...
class Node:
    def __init__(self, value, color, parent, data=None, left=None, right=None):
        self.value = value
        self.color = color
        if data is not None:
            self.data: Set = {data}
...
class RedBlackTree:
    ...
    NIL_LEAF = Node(value=None, color=NIL, parent=None, data=None)
    ...
    def add(self, value, data):
        if not self.root:
            self.root = Node(value, color=BLACK, parent=None, data=data, left=self.NIL_LEAF, right=self.NIL_LEAF)
            self.count += 1
            return
        parent, node_dir = self._find_parent(value)
        if node_dir is None:
            self.find_node(value).data.add(data)
            return  # value is in the tree
        new_node = Node(value=value, color=RED, parent=parent, data=data, left=self.NIL_LEAF, right=self.NIL_LEAF)
        if node_dir == 'L':
    ...
    def remove(self, value, data=None):
        """
        ...
        to_return = None
        if data is not None:
            node_to_remove.data.discard(data)
        else:
            to_return = node_to_remove.data.pop()
        if len(node_to_remove.data) == 0:
    ...
    def _try_rebalance(self, node):
        ...
        if uncle == self.NIL_LfEAF or uncle.color == BLACK:
            # rotate
